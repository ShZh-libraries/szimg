...
fn generate_huffman_table(huffman_spec: &HuffmanSpec) -> HashMap<u8, Bits> {
    let mut huffman_table = HashMap::new();

    let mut value_index = 0;
    let mut codeword = 0;
    for count_index in 0..huffman_spec.count.len() {
        for _ in 0..huffman_spec.count[count_index] {
            let key = huffman_spec.value[value_index];
            // count_index + 1 is the code length
            let value = Bits::new(count_index as u8 + 1, codeword);
            huffman_table.insert(key, value);
...
lazy_static! {
    ...
    pub static ref LUMINANCE_DC_TABLE: HashMap<u8, Bits> =
        generate_huffman_table(&LUMINANCE_DC_SPEC);
